...
with col4:
    tcv_reel_total = sum(
        float(c.get("TCV réel", 0) or 0) for c in contracts
    )
...
if filter_compliance == "Conformes":
    filtered_df = filtered_df[
        filtered_df['IFRS 15 AGI'] == 'Yes'
    ]

elif filter_compliance == "Non conformes":
    filtered_df = filtered_df[
        filtered_df['IFRS 15 AGI'] == 'No'
    ]
...
tab1, tab2 = st.tabs(
    ["Vue standard", "Vue financière ARR / TCV"]
)
...
with tab1:
    ...
    display_columns_standard = [
        c for c in display_columns_standard
        if c in filtered_df.columns
    ]

    st.dataframe(
        filtered_df[display_columns_standard],
        use_container_width=True,
        hide_index=True
    )
...
with tab2:
    ...
    display_columns_financial = [
        c for c in display_columns_financial
        if c in filtered_df.columns
    ]
    ...
    if len(filtered_df) > 0:
        ...
        total_arr = sum(
            float(c.get("ARR prix plein", 0) or 0)
            for c in contracts
            if c.get("Client Name")
            in filtered_df["Client Name"].values
        )

        total_tcv_th = sum(
            float(c.get("TCV théorique", 0) or 0)
            for c in contracts
            if c.get("Client Name")
            in filtered_df["Client Name"].values
        )
        ...
        with recap_cols[3]:
            st.metric(
                "Écart total (remises ramp-up)",
                f"{total_ecart:,.0f} €"
            )


st.caption(
    f"Affichage de {len(filtered_df)} contrat(s) "
    f"sur {len(df_contracts)}"
)
...
export_cols = [
    c for c in export_columns_order
    if c in filtered_df.columns
]

# Ajouter les colonnes restantes non listées
for col in filtered_df.columns:
    if col not in export_cols and col not in [
        'id',
        'timestamp',
        'reasoning',
        'sources'
...
with col3:
    ...
    st.download_button(
        ...
        "session_export.json",
        use_container_width=True,
        key="export_json_dash"
    )


# Export spécifique pour l'évaluation IFRS 15
with col4:

    from core.file_io import exporter_resultats

    if st.button(
        "Exporter pour évaluation",
        use_container_width=True
    ):

        chemin = exporter_resultats(contracts)

        st.success(
            f"Export créé : {chemin}"
        )
